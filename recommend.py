...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    cluster_pairs = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
    return group_by_first(cluster_pairs)
    # END Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    zipped_xy = zip([(x - mean_x) for x in xs], [(y - mean_y) for y in ys])
    Sxy = sum([x * y for x, y in zipped_xy])
    b = Sxy / Sxx
    a = mean_y - b * mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    best_predictor, _ = max([find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns], key=lambda x: x[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9 
    return {restaurant_name(r): (predictor(r) if r not in reviewed else user_rating(user, restaurant_name(r))) for r in restaurants}
    # END Question 9
